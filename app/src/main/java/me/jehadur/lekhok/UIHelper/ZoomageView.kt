...
class ZoomageView : AppCompatImageView, OnScaleGestureListener {
    ...
    private val currentDisplayedWidth: Float
        private get() = if (drawable != null) drawable.intrinsicWidth * matrixValues[Matrix.MSCALE_X] else 0F
    ...
    private val currentDisplayedHeight: Float
        private get() = if (drawable != null) drawable.intrinsicHeight * matrixValues[Matrix.MSCALE_Y] else 0F
    ...
    protected fun disallowParentTouch(event: MotionEvent?): Boolean {
        return if (currentPointerCount > 1 || currentScaleFactor > 1.0f || isAnimating) {
            true
        } else {
            false
        }
    ...
    private val isAnimating: Boolean
        private get() = resetAnimator != null && resetAnimator!!.isRunning
